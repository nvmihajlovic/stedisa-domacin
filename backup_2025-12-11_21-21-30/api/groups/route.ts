...
export async function GET(req: NextRequest) {
  try {
    ...
    // Add isOwner and canInvite flags
    const groupsWithPermissions = uniqueGroups.map((group: any) => {
    ...
    console.log(`📋 [groups] Returning ${groupsWithPermissions.length} groups for user ${user.userId}`);
...
export async function POST(req: NextRequest) {
  try {
    ...
    const group = await prisma.$transaction(async (tx) => {
      ...
      console.log(`👥 User ${user.userId} has ${userGroups} other groups`)

      // If this is the user's first group, set it as active
      if (userGroups === 0) {
        console.log(`🎯 Setting group ${newGroup.id} as active for user ${user.userId}`)
        await tx.user.update({
          where: { id: user.userId },
          data: { activeGroupId: newGroup.id }
        });
      } else {
        console.log(`⏭️ Not setting as active - user already has ${userGroups} groups`)
      }
