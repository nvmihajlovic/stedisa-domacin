...
export async function DELETE(
  ...
  if (income.groupId && income.group) {
    const currentUser = await prisma.user.findUnique({
      where: { id: user.userId },
      select: { name: true }
    })

    const otherMembers = income.group.members
      ...
      const notificationPromises = otherMembers.map(memberId => {
        return prisma.notification.create({
          data: {
            userId: memberId,
            type: 'INCOME_DELETED',
            title: 'Prihod obrisan iz grupe',
            message: `${currentUser?.name || 'Član grupe'} je obrisao prihod "${income.description}" (${income.amount.toFixed(2)} ${income.currency || 'RSD'}) iz grupe "${income.group?.name || 'Grupa'}"`,
            isRead: false,
...
export async function PUT(
  ...
  try {
    ...
    // Ako je income u grupi, pošalji notifikacije drugim članovima
    if (finalIncome?.groupId && finalIncome.group) {
      const currentUser = await prisma.user.findUnique({
        where: { id: user.userId },
        select: { name: true }
      })

      const otherMembers = finalIncome.group.members
        .filter(m => m.userId !== user.userId)
        .map(m => m.userId)
        ...
        const notificationPromises = otherMembers.map(memberId => {
          return prisma.notification.create({
            data: {
              userId: memberId,
              type: 'INCOME_UPDATED',
              title: 'Prihod ažuriran u grupi',
              message: `${currentUser?.name || 'Član grupe'} je ažurirao prihod "${finalIncome.description}" (${finalIncome.amount.toFixed(2)} ${finalIncome.currency || 'RSD'}) u grupi "${finalIncome.group?.name || 'Grupa'}"`,
              isRead: false,
              groupId: finalIncome.groupId,
              fromUserId: user.userId
            }
          })
        })
