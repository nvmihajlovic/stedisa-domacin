...
export async function POST(req: NextRequest) {
  try {
    ...
    console.log("📧 Email check:", {
      ...
      userEmailLower: dbUser?.email?.toLowerCase(),
    ...
    if (dbUser?.email?.toLowerCase().trim() !== invite.email.toLowerCase().trim()) {
      console.log("❌ Email mismatch - user must logout and login with correct email");
      return NextResponse.json(
        { 
