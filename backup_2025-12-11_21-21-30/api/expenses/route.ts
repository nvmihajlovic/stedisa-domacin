...
export async function POST(req: Request) {
  try {
    ...
    // Ako je trošak u grupi, pošalji notifikacije svim članovima osim trenutnog korisnika
    if (createdExpense?.groupId && createdExpense.group) {
      const currentUser = await prisma.user.findUnique({
        where: { id: user.userId },
        select: { name: true }
      })

      const otherMembers = createdExpense.group.members
        .filter(m => m.userId !== user.userId)
        .map(m => m.userId)
      
      console.log(`📢 Sending expense notifications to ${otherMembers.length} group members`)
      ...
      const notificationPromises = otherMembers.map(memberId => {
        return prisma.notification.create({
          data: {
            userId: memberId,
            type: 'EXPENSE_ADDED',
            title: 'Novi trošak u grupi',
            message: `${currentUser?.name || 'Član grupe'} je dodao trošak "${createdExpense.description}" (${createdExpense.amount.toFixed(2)} ${createdExpense.currency || 'RSD'}) u grupu "${createdExpense.group?.name || 'Grupa'}"`,
            isRead: false,
      ...
      await Promise.all(notificationPromises)
      console.log(`✅ Sent ${otherMembers.length} expense notifications`)
    }
