...
export default function IncomesPage() {
  ...
  const fetchIncomes = async () => {
    const res = await fetch(`/api/incomes?month=${month}&year=${year}&groupFilter=all`);
    if (res.ok) {
  ...
  const confirmDelete = async () => {
    ...
    const res = await fetch(`/api/incomes/${incomeToDelete}`, {
      method: "DELETE",
    });
          ...
          <div className="rounded-3xl backdrop-blur-xl p-6 lg:p-7 pb-8" style={{background: 'rgba(20, 18, 38, 0.85)', border: '1px solid rgba(255, 255, 255, 0.08)'}}>
            ...
            <div className="p-3 rounded-[16px]" style={{background: 'rgba(0, 0, 0, 0.4)', border: '1px solid rgba(255, 255, 255, 0.06)'}}>
              <button
                ...
                style={{background: theme.income.gradient, border: `1px solid ${theme.income.border}`, color: theme.income.primary, fontFamily: '"Inter", "Poppins", sans-serif', fontWeight: 600}}
              >
                + Dodaj prihod
              </button>
            </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50" onClick={handleCancelEdit}>
          <div className="w-full max-w-lg p-8 rounded-2xl max-h-[90vh] overflow-y-auto relative" style={{background: '#242236'}} onClick={(e) => e.stopPropagation()}>
            ...
            <form onSubmit={handleUpdate} className="space-y-4">
              <div>
                <label className="block text-sm mb-2" style={{color: '#A5A4B6'}}>Naziv prihoda *</label>
                <input type="text" value={description} onChange={(e) => setDescription(e.target.value)} placeholder="npr. Plata..." className="w-full px-4 py-3 rounded-xl outline-none" style={{background: '#1C1A2E', border: '1px solid #2E2B44', color: '#fff'}} />
              </div>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-3 md:gap-4">
                <div className="md:col-span-2">
                  <label className="block text-sm mb-2" style={{color: '#A5A4B6'}}>Iznos *</label>
                  <input type="number" step="0.01" value={amount} onChange={(e) => setAmount(e.target.value)} className="w-full px-4 py-3 rounded-xl outline-none" style={{background: '#1C1A2E', border: '1px solid #2E2B44', color: '#fff'}} />
                </div>
                <div>
                  <label className="block text-sm mb-2" style={{color: '#A5A4B6'}}>Valuta</label>
              ...
              <div>
                <label className="block text-sm mb-2" style={{color: '#A5A4B6'}}>Datum *</label>
                <input type="date" value={date} onChange={(e) => setDate(e.target.value)} className="w-full px-4 py-3 rounded-xl outline-none" style={{background: '#1C1A2E', border: '1px solid #2E2B44', color: '#fff'}} />
              </div>
              <div>
                <label className="block text-sm mb-2" style={{color: '#A5A4B6'}}>Kategorija *</label>
                <CategorySelector
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50" onClick={() => setShowDeleteModal(false)}>
          <div className="w-full max-w-md p-8 rounded-2xl relative" style={{background: '#242236'}} onClick={(e) => e.stopPropagation()}>
            ...
            <div className="text-center mb-6">
              ...
              <p style={{color: '#A5A4B6'}}>Prihod će biti trajno obrisan i neće moći da se vrati.</p>
            </div>
        ...
        >
          ...
          >
            <div className="flex flex-col items-center text-center gap-4">
              <div className="w-16 h-16 rounded-full flex items-center justify-center" style={{background: "rgba(31, 191, 164, 0.15)"}}>
                <Trash size={32} weight="duotone" style={{color: "#1FBFA4"}} />
              </div>
              <div>
                <h3 className="text-xl font-bold mb-2" style={{color: "#FFFFFF"}}>
                  Obrisati {selectedIncomes.size} prihoda?
                </h3>
                <p className="text-sm" style={{color: "#FFFFFF", opacity: 0.7}}>
                  Svi izabrani prihodi će biti trajno obrisani i neće moći da se vrate.
                </p>
              </div>
              <div className="flex gap-3 w-full mt-2">
                <button
                  ...
                  style={{background: "#1FBFA4", color: "#FFFFFF"}}
                >
                  Obriši sve
                </button>
                <button
