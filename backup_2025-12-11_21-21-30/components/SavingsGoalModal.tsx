  ...
  const fetchMyGroups = async () => {
    try {
      ...
      if (res.ok) {
        ...
        console.log('💼 [SavingsGoalModal] Fetched groups:', groups.length)
        
        // Filter groups where user can create savings goals (owner or has permission)
        const eligibleGroups = groups.filter((g: any) => g.isOwner || g.canInvite)
        setMyGroups(eligibleGroups)
        console.log('✅ [SavingsGoalModal] Eligible groups:', eligibleGroups.length)
      }
    } catch (error) {
    ...
    >
      ...
      >
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          {/* Group Selection - samo kada je grupna štednja */}
          {isGroup && (
            <div>
              <label htmlFor="group-select-savings" style={{ color: 'rgba(255, 255, 255, 0.7)', fontSize: '14px', marginBottom: '12px', display: 'block' }}>
                Izaberi grupu
              </label>
              <select
                id="group-select-savings"
