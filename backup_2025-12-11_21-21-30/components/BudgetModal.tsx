...
export default function BudgetModal({
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      if (editingBudget) {
        ...
        setSelectedGroupId(editingBudget.groupId || '');
      } else {
        setCategoryId('');
        setAmount('');
        setPeriod('MONTHLY');
  ...
  }, [isOpen, editingBudget, activeGroupId]);
  ...
  const fetchMyGroups = async () => {
    try {
      ...
      if (res.ok) {
        ...
        console.log('💼 [BudgetModal] Fetched groups:', groups.length);
        
        // Filter groups where user can create budgets (owner or has permission)
        const eligibleGroups = groups.filter((g: any) => g.isOwner || g.canInvite);
        setMyGroups(eligibleGroups);
        
        // Auto-select active group if not editing
        if (!editingBudget && activeGroupId && eligibleGroups.some((g: any) => g.id === activeGroupId)) {
          setSelectedGroupId(activeGroupId);
          console.log('✅ [BudgetModal] Auto-selected active group:', activeGroupId);
        }
      }
    } catch (error) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (budgetType === 'group' && selectedGroupId) {
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50" onClick={onClose}>
      <div className="w-full max-w-lg p-8 rounded-2xl max-h-[90vh] overflow-y-auto relative" style={{background: '#242236'}} onClick={(e) => e.stopPropagation()}>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          {/* Group Selection - samo kada je grupni tip budžeta */}
          {budgetType === 'group' && !editingBudget && (
            <div className="p-4 rounded-xl space-y-3" style={{background: 'rgba(159, 112, 255, 0.05)', border: '1px solid rgba(159, 112, 255, 0.15)'}}>
              <label htmlFor="group-select" className="block text-sm font-medium" style={{color: '#A5A4B6'}}>
                Izaberi grupu
              </label>
              <select
                id="group-select"
                ...
                style={{
                  background: '#2A2838',
                  borderColor: 'rgba(159, 112, 255, 0.3)',
                  color: '#FFFFFF'
                }}
                required
              >
                ...
                {myGroups.map((group) => (
                  <option key={group.id} value={group.id}>
                    ...
                    {group.id === activeGroupId ? ' (Aktivna)' : ''}
                  </option>
                ))}
              </select>
            </div>
