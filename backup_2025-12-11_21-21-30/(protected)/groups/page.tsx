...
export default function GroupsPage() {
  ...
  const fetchMyGroups = async () => {
    try {
      console.log("🔍 [fetchMyGroups] Starting...");
      const res = await fetch("/api/groups");
      if (res.ok) {
        const data = await res.json();
        console.log("📦 [fetchMyGroups] Received data:", data);
        
        // API vraća direktno niz grupa, ne objekat sa groups property
        const allGroups = Array.isArray(data) ? data : (data.groups || []);
        console.log("📋 [fetchMyGroups] All groups:", allGroups.length);
        
        // Filter only groups where user can invite
        const invitableGroups = allGroups.filter((g: Group) => {
          const canInvite = g.isOwner || g.canInvite;
          console.log(`🔐 Group "${g.name}": isOwner=${g.isOwner}, canInvite=${g.canInvite}, result=${canInvite}`);
          return canInvite;
        });
        console.log("✅ [fetchMyGroups] Invitable groups:", invitableGroups.length);
        setMyGroups(invitableGroups);
        
        // Auto-select group: prefer active group, then current group, then first available
        if (activeGroupId && invitableGroups.some((g: Group) => g.id === activeGroupId)) {
          setSelectedGroupForInvite(activeGroupId);
          console.log("🎯 [fetchMyGroups] Selected active group:", activeGroupId);
        } else if (group && invitableGroups.some((g: Group) => g.id === group.id)) {
          setSelectedGroupForInvite(group.id);
          console.log("🎯 [fetchMyGroups] Selected current group:", group.id);
        } else if (invitableGroups.length > 0) {
          setSelectedGroupForInvite(invitableGroups[0].id);
          console.log("🎯 [fetchMyGroups] Selected first group:", invitableGroups[0].id);
        }
      } else {
        console.error("❌ [fetchMyGroups] API error:", res.status, res.statusText);
      }
    } catch (error) {
  ...
  const handleSendInvite = async () => {
    ...
    if (!selectedGroupForInvite) {
      addToast("Molimo izaberite grupu", "warning");
      return;
    }
        ...
        <div className="flex items-center justify-between mb-8" style={{ paddingTop: "24px" }}>
          ...
          <div className="flex items-center gap-3">
            ...
            {group.canInvite && (
              <button
                onClick={() => {
                  fetchMyGroups(); // Osvezi listu grupa
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <select
                      ...
                      className="w-full px-4 py-3 bg-[#0f1015] border border-white/10 rounded-xl text-white focus:border-purple-500 focus:outline-none appearance-none cursor-pointer"
                    >
                      <option value="" disabled>Izaberite grupu...</option>
                      {myGroups.length === 0 && (
                        <option disabled>Učitavanje grupa...</option>
                      )}
                      {myGroups.map((g) => (
                        <option key={g.id} value={g.id}>
                          {g.name} {g.type === "TEMPORARY" ? "🕒" : ""}
                          {g.id === activeGroupId ? " (Aktivna)" : ""}
                        </option>
                      ))}
                    </select>
                  </div>
