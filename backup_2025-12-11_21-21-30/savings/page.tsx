...
export default function SavingsPage() {
  ...
  const [deleteConfirm, setDeleteConfirm] = useState<{ show: boolean; goalId: string | null; goalName: string | null }>({
    show: false,
    goalId: null,
    goalName: null
  })
  ...
  const handleDeleteGoal = async (goalId: string) => {
    try {
      ...
      if (response.ok && data.success) {
        showToast(data.message || 'Cilj štednje uspešno obrisan', 'success')
        setDeleteConfirm({ show: false, goalId: null, goalName: null })
        fetchSavingsGoals()
      } else {
      ...
      showToast('Greška prilikom brisanja cilja: ' + (error.message || 'Nepoznata greška'), 'error')
    }
  }

  const handleDeleteClick = (goalId: string, goalName: string) => {
    setDeleteConfirm({ show: true, goalId, goalName })
  }
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#0A0A0F' }}>
      ...
      {deleteConfirm.show && (
        ...
        >
          <div 
            className="bg-[#1C1A2E] rounded-2xl p-8 max-w-md w-full border border-white/10"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="text-center">
              <div className="w-16 h-16 mx-auto mb-4 rounded-full bg-red-500/10 flex items-center justify-center">
                <Trash size={32} weight="duotone" style={{ color: '#ef4444' }} />
              </div>
              ...
              <p className="text-white/60 mb-6">
                Da li ste sigurni da želite da obrišete cilj <span className="text-white font-semibold">"{deleteConfirm.goalName}"</span>?
              ...
              <div className="flex gap-3">
                <button
                  onClick={cancelDelete}
                  className="flex-1 py-3 rounded-xl font-semibold text-white transition-all hover:bg-white/5"
                  style={{ background: '#242236', border: '1px solid rgba(255, 255, 255, 0.1)' }}
                >
                  Otkaži
                </button>
                <button
