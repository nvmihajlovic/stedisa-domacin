...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    // Get group creation date for temporal filtering
    ...
    console.log(`🔍 [member-stats] Fetching transactions for member ${memberId} in group ${groupId}`)
    console.log(`📅 [member-stats] Group created at: ${group.createdAt}`)

    // Fetch ONLY expenses that belong to this group AND were created AFTER group creation
    const expenses = await prisma.expense.findMany({
      where: {
        userId: memberId,
        groupId: groupId,  // ✅ Only expenses in this group
        createdAt: {
          gt: group.createdAt  // ✅ Only expenses created AFTER group creation
        }
      },
      include: {
        category: true,
    ...
    console.log(`📊 [member-stats] Found ${expenses.length} expenses for member ${memberId}`)

    // Fetch ONLY incomes that belong to this group AND were created AFTER group creation
    const incomes = await prisma.income.findMany({
      where: {
        userId: memberId,
        groupId: groupId,  // ✅ Only incomes in this group
        createdAt: {
          gt: group.createdAt  // ✅ Only incomes created AFTER group creation
        }
      },
      include: {
        category: true,
    ...
    console.log(`💰 [member-stats] Found ${incomes.length} incomes for member ${memberId}`)
