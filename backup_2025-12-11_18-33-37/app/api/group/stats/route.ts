...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    console.log(`🔍 [group-stats] Fetching transactions for group ${group.id}`);
    console.log(`📅 [group-stats] Group created at: ${group.createdAt}`);
    ...
    const expensePromises = activeMembers.map((member: any) => {
      ...
      return prisma.expense.findMany({
        where: {
          ...
          groupId: group.id,  // ✅ Only expenses in this group
          createdAt: {
    ...
    const incomePromises = activeMembers.map((member: any) => {
      ...
      return prisma.income.findMany({
        where: {
          userId: member.userId,
          groupId: group.id,  // ✅ Only incomes in this group
          createdAt: {
    ...
    console.log(`📊 [group-stats] Found ${expenses.length} expenses`);
    console.log(`💰 [group-stats] Found ${incomes.length} incomes`);
