...
export default function DashboardClient({ user }: { user: User }) {
  ...
  const [expSavingsGoalId, setExpSavingsGoalId] = useState("")
  ...
  // Fetch savings goals when Štednja category is selected
  useEffect(() => {
    console.log('⚡ useEffect triggered! expCategoryId:', expCategoryId, 'categories.length:', categories.length)
    if (expCategoryId) {
      const selectedCategory = categories.find(c => c.id === expCategoryId)
      console.log('🔍 Selected category:', selectedCategory?.name, 'isSavings:', selectedCategory?.isSavings)
      if (selectedCategory?.isSavings) {
        console.log('💰 Fetching available savings goals...')
    ...
    const checkIfNewUser = async () => {
      try {
        ...
        const [expensesRes, incomesRes] = await Promise.all([
          fetch("/api/expenses?groupFilter=all"),
          fetch("/api/incomes?groupFilter=all")
        ])
  ...
  const fetchCategories = async () => {
    ...
    if (catRes.ok) {
      ...
      const activeCategories = data.filter((cat: any) => cat.isActive !== false)
      console.log('📚 Loaded categories:', activeCategories.map((c: any) => ({ name: c.name, isSavings: c.isSavings })))
  ...
  const fetchStats = async () => {
    ...
    const [monthlyExpensesRes, monthlyIncomesRes, totalExpensesRes, totalIncomesRes] = await Promise.all([
      fetch(`/api/expenses?month=${month}&year=${year}&groupFilter=all`),
      fetch(`/api/incomes?month=${month}&year=${year}&groupFilter=all`),
      fetch(`/api/expenses?groupFilter=all`),
      fetch(`/api/incomes?groupFilter=all`)
    ])
    ...
    if (editMode === 'expense' && editingId) {
      ...
      const res = await fetch("/api/expenses", {
        ...
        body: JSON.stringify({
          ...
          recurringType: expIsRecurring ? expRecurringType : null,
          nextRecurringDate: expIsRecurring ? expNextRecurringDate : null,
          savingsGoalId: expSavingsGoalId || null
        })
      })
      if (res.ok) {
        setShowExpenseModal(false)
  ...
  const handleIncomeSubmit = async (e: React.FormEvent) => {
    ...
    if (!incDescription.trim()) {
      console.warn('Molimo unesite opis prihoda')
      return
    }
    if (!incAmount || parseFloat(incAmount) <= 0) {
      console.warn('Molimo unesite ispravan iznos')
      return
    }
    if (!incDate) {
                      ...
                      <svg width="100%" height="100%" viewBox="0 0 100 100" className="drop-shadow-sm">
                        ...
                        <g transform="rotate(-90 50 50)">
                          {(() => {
                            ...
                            console.log('🎨 [PIE-RENDER] Rendering expense pie segments:', expensesByCategory.length);
                            let currentAngle = 0;
                            return expensesByCategory.map((cat, idx) => {
                              console.log(`🎨 [SEGMENT-${idx}] Rendering:`, cat.name, cat.total);
                              ...
                              const handleMouseEnter = (e: React.MouseEvent<SVGPathElement>) => {
                                console.log('🎯 Mouse enter on expense pie:', cat.name);
                    ...
                    {recentExpenses.map((expense, idx) => {
                      ...
                      >
                        <div className="flex items-center gap-3 flex-1 cursor-pointer" onClick={(e) => { e.stopPropagation(); router.push('/expenses'); }}>
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-2 flex-wrap">
                              ...
                              <div className="flex-shrink-0 px-2 py-0.5 rounded-md text-xs font-medium" style={{background: `${categoryColor}15`, border: `1px solid ${categoryColor}30`, color: categoryColor}}>
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          >
                            <PencilSimple size={16} weight="duotone" style={{color: categoryColor}} />
                          </button>
                        </div>
                      </div>
                    );
                ...
                {recentIncomes.length > 0 ? (
                  <>
                    {recentIncomes.map((income, idx) => {
                      ...
                      const categoryColor = income.category?.color || '#45D38A';
                      return (
                        <div 
                        key={income.id}
                        className="rounded-xl p-4 flex items-center justify-between bg-white/5 transition-all duration-[120ms] ease-out hover:brightness-[1.02] hover:scale-[1.01]"
                      ...
                      >
                        <div className="flex items-center gap-3 flex-1 cursor-pointer" onClick={(e) => { e.stopPropagation(); router.push('/incomes'); }}>
                          <div className="w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0" style={{background: `${categoryColor}25`, border: `1px solid ${categoryColor}40`, boxShadow: `0 0 15px ${categoryColor}40`}}>
                            <IconComponent size={18} weight="duotone" style={{color: categoryColor}} />
                          </div>
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-2 flex-wrap">
        ...
        <div className="fixed flex flex-col gap-4 left-1/2 -translate-x-1/2 lg:left-auto lg:translate-x-0 lg:right-8" style={{bottom: '32px'}} data-tour="add-expense">
          <button
            ...
            style={{
              ...
              boxShadow: `0 0 10px ${theme.expense.shadow}, 0 4px 16px rgba(0, 0, 0, 0.3)`,
              border: `1px solid ${theme.neutral.borderLight}`,
              transform: 'scale(1)',
            }}
            onMouseEnter={(e) => {
              e.currentTarget.style.transform = 'scale(1.05)';
              e.currentTarget.style.background = theme.expense.gradientSolid;
              e.currentTarget.style.boxShadow = `0 0 16px ${theme.expense.shadow}, 0 6px 20px rgba(0, 0, 0, 0.4)`;
            }}
            onMouseLeave={(e) => {
              e.currentTarget.style.transform = 'scale(1)';
              e.currentTarget.style.background = theme.expense.gradientBright;
              e.currentTarget.style.boxShadow = `0 0 10px ${theme.expense.shadow}, 0 4px 16px rgba(0, 0, 0, 0.3)`;
            }}
          >
          ...
          <button
            ...
            className="w-[60px] h-[60px] rounded-full transition-all duration-300 flex items-center justify-center group relative"
            style={{
              background: theme.income.gradientBright,
              boxShadow: `0 0 10px ${theme.income.shadow}, 0 4px 16px rgba(0, 0, 0, 0.3)`,
              border: `1px solid ${theme.neutral.borderLight}`,
              transform: 'scale(1)',
            }}
            onMouseEnter={(e) => {
              e.currentTarget.style.transform = 'scale(1.05)';
              e.currentTarget.style.background = theme.income.gradientSolid;
              e.currentTarget.style.boxShadow = `0 0 16px ${theme.income.shadow}, 0 6px 20px rgba(0, 0, 0, 0.4)`;
            }}
            onMouseLeave={(e) => {
              e.currentTarget.style.transform = 'scale(1)';
              e.currentTarget.style.background = theme.income.gradientBright;
              e.currentTarget.style.boxShadow = `0 0 10px ${theme.income.shadow}, 0 4px 16px rgba(0, 0, 0, 0.3)`;
            }}
          >
        ...
        {showExpenseModal && (
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50" onClick={() => setShowExpenseModal(false)}>
            <div className="w-full max-w-lg p-8 rounded-2xl max-h-[90vh] overflow-y-auto relative" style={{background: '#242236'}} onClick={(e) => e.stopPropagation()}>
              ...
              <h2 className="text-2xl font-bold mb-6" style={{background: '#FFD700', color: '#000', padding: '8px'}}>
                🚀 TEST v3.0 FORCE REBUILD - Cat: {expCategoryId || 'NONE'}
              </h2>
              ...
              <form onSubmit={handleExpenseSubmit} className="space-y-4">
                ...
                <div>
                  ...
                  {/* VISIBLE DEBUG */}
                  <div style={{background: 'red', color: 'white', padding: '10px', fontSize: '14px', fontWeight: 'bold', marginBottom: '10px'}}>
                    ⚠️ DEBUG VISIBLE: expCategoryId = "{expCategoryId || 'PRAZNO'}"
                  </div>
                  
                  <input type="text" value={expDescription} onChange={(e) => setExpDescription(e.target.value)} placeholder="npr. Kupovina..." className="w-full px-4 py-3 rounded-xl outline-none" style={{background: '#1C1A2E', border: '1px solid #2E2B44', color: '#fff'}} />
                </div>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-3 md:gap-4">
                  <div className="md:col-span-2">
                    <label className="block text-sm mb-2" style={{color: '#A5A4B6'}}>Iznos *</label>
                    <input type="number" step="0.01" value={expAmount} onChange={(e) => setExpAmount(e.target.value)} className="w-full px-4 py-3 rounded-xl outline-none" style={{background: '#1C1A2E', border: '1px solid #2E2B44', color: '#fff'}} />
                  </div>
                  <div>
                ...
                <div>
                  ...
                  <CategorySelector
                    ...
                    onSelect={(id) => {
                      console.log('🎯 Category clicked! ID:', id, 'Category:', categories.find(c => c.id === id))
                      setExpCategoryId(id)
                    }}
                  />
                </div>
                ...
                <div className="p-2 mb-2 text-xs" style={{background: '#2E2B44', color: '#FFD700', borderRadius: '8px'}}>
                  DEBUG: expCategoryId = {expCategoryId || 'PRAZNO'} | 
                  Category name = {expCategoryId ? categories.find(c => c.id === expCategoryId)?.name || 'NOT FOUND' : 'NONE'} |
                  isSavings = {expCategoryId ? String(categories.find(c => c.id === expCategoryId)?.isSavings) : 'N/A'}
                </div>

                {/* Savings Goal Selector - only show if Štednja category is selected */}
                {expCategoryId && categories.find(c => c.id === expCategoryId)?.isSavings && (
                  <div className="rounded-xl p-4" style={{background: 'rgba(255, 215, 0, 0.05)', border: '1px solid rgba(255, 215, 0, 0.15)'}}>
                    ...
                    {availableSavingsGoals.length === 0 ? (
                      ...
                      <>
                        ...
                        <select
                          value={expSavingsGoalId}
                          onChange={(e) => setExpSavingsGoalId(e.target.value)}
                ...
                <div className="flex gap-4 pt-2">
                  ...
                  <button type="submit" className="flex-1 py-3 rounded-xl font-semibold text-white" style={{background: theme.expense.gradientSolid}}>Dodaj</button>
                </div>
              </form>
            </div>
          </div>
            ...
            <div className="w-full max-w-lg p-8 rounded-2xl max-h-[90vh] overflow-y-auto relative" style={{background: '#242236'}} onClick={(e) => e.stopPropagation()}>
              ...
              <form onSubmit={handleIncomeSubmit} className="space-y-4">
                <div>
                  ...
                  <input type="text" value={incDescription} onChange={(e) => setIncDescription(e.target.value)} placeholder="npr. Plata..." className="w-full px-4 py-3 rounded-xl outline-none" style={{background: '#1C1A2E', border: '1px solid #2E2B44', color: '#fff'}} />
                </div>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-3 md:gap-4">
                  <div className="md:col-span-2">
                    <label className="block text-sm mb-2" style={{color: '#A5A4B6'}}>Iznos *</label>
                    <input type="number" step="0.01" value={incAmount} onChange={(e) => setIncAmount(e.target.value)} className="w-full px-4 py-3 rounded-xl outline-none" style={{background: '#1C1A2E', border: '1px solid #2E2B44', color: '#fff'}} />
                  </div>
                  <div>
                    <label className="block text-sm mb-2" style={{color: '#A5A4B6'}}>Valuta</label>
                ...
                <div className="flex gap-4 pt-2">
                  ...
                  <button type="submit" className="flex-1 py-3 rounded-xl font-semibold text-white" style={{background: theme.income.gradientSolid}}>Dodaj</button>
                </div>
              </form>
            </div>
          </div>
          ...
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50" onClick={() => setShowEditExpenseModal(false)}>
            <div className="w-full max-w-lg p-8 rounded-2xl max-h-[90vh] overflow-y-auto relative" style={{background: '#242236'}} onClick={(e) => e.stopPropagation()}>
              ...
              <form onSubmit={handleUpdateExpense} className="space-y-4">
                ...
                <div>
                  <label className="block text-sm mb-2" style={{color: '#A5A4B6'}}>Datum *</label>
                  <input type="date" value={expDate} onChange={(e) => setExpDate(e.target.value)} className="w-full px-4 py-3 rounded-xl outline-none" style={{background: '#1C1A2E', border: '1px solid #2E2B44', color: '#fff'}} />
                </div>
                <div>
                  <label className="block text-sm mb-2" style={{color: '#A5A4B6'}}>Kategorija *</label>
                  <CategorySelector
                ...
                <div className="flex gap-4 pt-2">
                  ...
                  <button type="submit" className="flex-1 py-3 rounded-xl font-semibold text-white" style={{background: theme.expense.gradientSolid}}>Sačuvaj</button>
                </div>
              </form>
            </div>
          </div>
          ...
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50" onClick={() => setShowEditIncomeModal(false)}>
            <div className="w-full max-w-lg p-8 rounded-2xl max-h-[90vh] overflow-y-auto relative" style={{background: '#242236'}} onClick={(e) => e.stopPropagation()}>
              ...
              <form onSubmit={handleUpdateIncome} className="space-y-4">
                ...
                <div>
                  <label className="block text-sm mb-2" style={{color: '#A5A4B6'}}>Datum *</label>
                  <input type="date" value={incDate} onChange={(e) => setIncDate(e.target.value)} className="w-full px-4 py-3 rounded-xl outline-none" style={{background: '#1C1A2E', border: '1px solid #2E2B44', color: '#fff'}} />
                </div>
                <div>
                  <label className="block text-sm mb-2" style={{color: '#A5A4B6'}}>Kategorija *</label>
                  <CategorySelector
                ...
                <div className="flex gap-4 pt-2">
                  ...
                  <button type="submit" className="flex-1 py-3 rounded-xl font-semibold text-white" style={{background: theme.income.gradientSolid}}>Sačuvaj</button>
                </div>
              </form>
            </div>
          </div>
