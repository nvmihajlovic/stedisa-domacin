...
import { 
  ArrowLeft, Plus, Trash, Tag, PencilSimple, Eye, EyeSlash, 
  MagnifyingGlass, SortAscending, SortDescending, Funnel 
} from "phosphor-react";
...
export default function CategoriesPage() {
  ...
  // New states for filtering, sorting, and search
  ...
  const filteredAndSortedCategories = useMemo(() => {
    ...
    if (searchQuery.trim()) {
      ...
      expenses = expenses.filter(cat => cat.name.toLowerCase().includes(query));
      incomes = incomes.filter(cat => cat.name.toLowerCase().includes(query));
    }

    // Apply sorting
    const sortFn = (a: Category | IncomeCategory, b: Category | IncomeCategory) => {
  ...
  const handleDeleteCategory = async (category: any, type: 'expense' | 'income') => {
    try {
      const usage = await checkCategoryUsage(category.id, type);
      
      setDeletingCategory({ ...category, type });
      setDeleteHasTransactions(usage.hasTransactions);
      setDeleteTransactionCount(usage.count);
      setShowDeleteModal(true);
    } catch (error) {
      console.error("Error checking category usage:", error);
      showToast("Greška pri proveri kategorije", "error");
    }
  };
  ...
  const handleToggleActive = async (category: any, type: 'expense' | 'income') => {
    try {
      const newActiveState = !(category.isActive !== false);
      
      if (type === 'expense') {
        await updateCategory(category.id, { isActive: newActiveState });
        mutateExpenses();
      } else {
        await updateIncomeCategory(category.id, { isActive: newActiveState });
        mutateIncomes();
      }
      
      showToast(
        newActiveState ? "Kategorija aktivirana" : "Kategorija deaktivirana",
        "success"
      );
    } catch (error) {
  ...
  if (loading) {
    return (
      <div className="min-h-screen flex items-center justify-center" style={{ background: "#0A0817" }}>
        <div className="text-center">
          <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-purple-500 mx-auto mb-4"></div>
          <p style={{ color: "rgba(255, 255, 255, 0.6)" }}>Učitavanje...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen p-4 md:p-8" style={{ background: "#0A0817" }}>
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-8">
          ...
          <div className="flex items-center gap-3">
            <ModernHelpButton page="categories" />
        ...
        <div className="mb-8 p-6 rounded-2xl" 
          style={{
            background: 'linear-gradient(145deg, rgba(30, 27, 42, 0.6), rgba(23, 20, 33, 0.6))',
          ...
          <div className="flex flex-col lg:flex-row gap-4 items-stretch lg:items-center">
            ...
            >
              <button
                ...
                style={{
                  background: filterType === "all" ? 'rgba(167, 139, 250, 0.2)' : 'transparent',
                  color: filterType === "all" ? '#A78BFA' : 'rgba(255, 255, 255, 0.6)',
                  border: filterType === "all" ? '1px solid rgba(167, 139, 250, 0.3)' : '1px solid transparent'
                }}
              >
                Sve
              </button>
              <button
                ...
                className="px-4 py-2 rounded-lg font-semibold text-sm transition-all duration-200"
                style={{
                  background: filterType === "expenses" ? 'rgba(255, 107, 157, 0.2)' : 'transparent',
                  color: filterType === "expenses" ? '#FF6B9D' : 'rgba(255, 255, 255, 0.6)',
                  border: filterType === "expenses" ? '1px solid rgba(255, 107, 157, 0.3)' : '1px solid transparent'
                }}
              >
                Troškovi
              </button>
              <button
                ...
                className="px-4 py-2 rounded-lg font-semibold text-sm transition-all duration-200"
                style={{
                  background: filterType === "incomes" ? 'rgba(69, 211, 138, 0.2)' : 'transparent',
                  color: filterType === "incomes" ? '#45D38A' : 'rgba(255, 255, 255, 0.6)',
                  border: filterType === "incomes" ? '1px solid rgba(69, 211, 138, 0.3)' : '1px solid transparent'
                }}
              >
                Prihodi
              </button>
            ...
            <button
              ...
              className="px-5 py-3 rounded-xl flex items-center gap-2 font-semibold text-sm transition-all duration-200"
              style={{
              ...
              onMouseEnter={(e) => {
                e.currentTarget.style.background = 'rgba(77, 181, 255, 0.25)';
              }}
              onMouseLeave={(e) => {
                e.currentTarget.style.background = 'rgba(77, 181, 255, 0.15)';
              }}
            >
              {sortOrder === "asc" ? (
                <><SortAscending size={18} weight="bold" /> A–Z</>
              ) : (
                <><SortDescending size={18} weight="bold" /> Z–A</>
              )}
            </button>
          </div>

          {/* Results count */}
          <div className="mt-4 flex items-center gap-4 text-sm" style={{ color: 'rgba(255, 255, 255, 0.5)' }}>
            {shouldShowExpenses && (
              <span>
                <span style={{ color: '#FF6B9D', fontWeight: 600 }}>{filteredAndSortedCategories.expenses.length}</span> troškovi
              </span>
            )}
            {shouldShowIncomes && (
              <span>
                <span style={{ color: '#45D38A', fontWeight: 600 }}>{filteredAndSortedCategories.incomes.length}</span> prihodi
              </span>
            )}
            {searchQuery && (
          ...
          <div className="mb-12">
            <div className="flex items-center justify-between mb-6">
              ...
              <span className="text-sm px-3 py-1 rounded-lg" style={{ 
                ...
                {filteredAndSortedCategories.expenses.length} kategorija
              </span>
            </div>

            {filteredAndSortedCategories.expenses.length === 0 ? (
              ...
              >
                <p style={{ color: 'rgba(255, 255, 255, 0.4)' }}>
                  {searchQuery ? "Nema rezultata pretrage" : "Nema kategorija troškova"}
                </p>
              </div>
            ) : (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                {filteredAndSortedCategories.expenses.map((category, index) => {
                        ...
                        <div className="flex items-center justify-between mb-3">
                          ...
                          <div className="flex gap-1.5">
                            <button
                              ...
                              onMouseEnter={(e) => {
                                e.currentTarget.style.background = 'rgba(77, 181, 255, 0.2)';
                                e.currentTarget.style.borderColor = 'rgba(77, 181, 255, 0.4)';
                              }}
                              onMouseLeave={(e) => {
                                e.currentTarget.style.background = 'rgba(77, 181, 255, 0.1)';
                                e.currentTarget.style.borderColor = 'rgba(77, 181, 255, 0.2)';
                              }}
                            >
                              <PencilSimple size={14} weight="duotone" style={{ color: '#4DB5FF' }} />
                            </button>
          ...
          <div className="mb-12">
            <div className="flex items-center justify-between mb-6">
              ...
              <span className="text-sm px-3 py-1 rounded-lg" style={{ 
                ...
                {filteredAndSortedCategories.incomes.length} kategorija
              </span>
            </div>

            {filteredAndSortedCategories.incomes.length === 0 ? (
              <div className="text-center py-12 rounded-2xl" 
                style={{
                  background: 'rgba(255, 255, 255, 0.02)',
                  border: '1px solid rgba(255, 255, 255, 0.05)'
              ...
              >
                <p style={{ color: 'rgba(255, 255, 255, 0.4)' }}>
                  {searchQuery ? "Nema rezultata pretrage" : "Nema kategorija prihoda"}
                </p>
              </div>
            ) : (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                {filteredAndSortedCategories.incomes.map((category, index) => {
                  const IconComponent = getIcon(category.icon);
                  const isActive = category.isActive !== false;
